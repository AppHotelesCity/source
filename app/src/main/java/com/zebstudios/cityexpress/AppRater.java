...
/**
 * Created by usuario on 23/06/15.
 */
...
public class AppRater {
    ...
    public AlertDialog show() {
        ...
        if (mContext.getPackageManager().queryIntentActivities(mTargetIntent, 0).size() <= 0) {
            Log.e("AppRater","no app available to handle the intent");
        ...
        if (prefs.getBoolean(mPreference_dontShow, false)) { // if user opted not to rate the app
            Log.e("AppRater","No mostrar nunca jamas la alerta");
        ...
        if (firstLaunchTime == 0) { // if not set yet
            ...
            Log.e("AppRater", "mPreference_firstlaucher - " +mPreference_firstLaunch);
        }

        savePreferences(editor);

        //Log.e("Apprater", "V - Veces lanzado : " + mLaunchesBeforePrompt);
        //Log.e("Apprater", "V - DIAS lanzado : " + mDaysBeforePrompt);

        Log.e("AppRater", "Contador : " + launch_count);
        Log.e("AppRater","Veces lanzado : " +launch_count  );
        //Log.e("AppRater","Priemra vez lanzado  : " +firstLaunchTime  );
        //Log.e("AppRater", "--No se que es :- " +System.currentTimeMillis()  );
        //Log.e("AppRater", "Operacion rara :- " +(firstLaunchTime + (mDaysBeforePrompt * DateUtils.DAY_IN_MILLIS)) );
        ...
        if (launch_count >= mLaunchesBeforePrompt) { // wait at least x app launches
            if (System.currentTimeMillis() >= (firstLaunchTime + (mDaysBeforePrompt * DateUtils.DAY_IN_MILLIS))) { // wait at least x days
                try {
                    Log.e("AppRater","Alerta Mostrada correctamente :3");

                    editor.putLong(mPreference_launchCount, 0); // write new counter back to preference

                    return showDialog(mContext, editor, firstLaunchTime);
                }
                catch (Exception e) {
                    Log.e("AppRater","Error al lanzar la alerta");

                    return null;
                }
            }
            else {
                Log.e("AppRater","No se lanzo porque faltan dias para que se muestre la alerta");

                return null;
            }
        }
        else {
            Log.e("AppRater","Falta ser lanzada mas veces para mostrar alerta");
